            ...
            if (audio.length > 0) {
                ...
                if (link.length > 0) {
                    ...
                    audio.bind('click', function(e) {
                        var embedCode;							
						//if($scw.browser && $scw.browser.mozilla&&navigator.userAgent.indexOf("Trident")==-1){
                        if (typeof InstallTrigger !== 'undefined')
                        {
						    embedCode="<audio controls autoplay hidden><source src=\""+ $scw(this).attr('href') +"\" type=\"audio/mpeg\"></audio>";
						}
						else{
							embedCode= "<EMBED SRC=" + $scw(this).attr('href') + " HIDDEN='true' AUTOSTART='true' class=\"qwerty\" /></embed>";
						}
						//var embedCode="<audio controls autoplay ><source src=\""+ $scw(this).attr('href') +"\" type=\"audio/mpeg\"></audio>";
